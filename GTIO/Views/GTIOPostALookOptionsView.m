            ...
            if (on && !currentUser.isFacebookConnected) {
                [currentUser connectToFacebookWithLoginHandler:^(GTIOUser *user, NSError *error) {
                    if (!user.isFacebookConnected) {
                        GTIOAlertView *alert = [[GTIOAlertView alloc] initWithTitle:@"" message:@"We were unable to connect you with facebook." delegate:nil cancelButtonTitle:@"Okay" otherButtonTitles:nil];
                        [alert show];
