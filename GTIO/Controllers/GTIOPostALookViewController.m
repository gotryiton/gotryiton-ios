...
{
    if (buttonIndex == 1 && alertView.tag == kGTIOEmptyDescriptionAlertTag) {
        [self beginPostLookToGTIO];
    }
    if (buttonIndex == 1 && alertView.tag == kGTIOEmptyPostAlertTag) {
        [self cancelPost];
    }
}
...
- (void)setOriginalImage:(UIImage *)originalImage filteredImage:(UIImage *)filteredImage filterName:(NSString *)filterName productID:(NSNumber *)productID
{
    switch (self.currentSection) {
        case GTIOPostPhotoSectionMain:
            ...
            CGFloat ratio = self.mainFilteredImage.size.height / self.mainFilteredImage.size.width;
            CGFloat height = roundf(self.lookSelectorView.frame.size.width*ratio);
            height = MIN(kGTIOLookSelectorViewMaxHeight, height);
            height = MAX(kGTIOLookSelectorViewMinHeight, height);
