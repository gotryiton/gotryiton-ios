    ...
    GTIOUIButton *saveButton = [GTIOUIButton buttonWithGTIOType:GTIOButtonTypeSaveGrayTopMargin tapHandler:^(id sender) {
        ...
        [[GTIOUser currentUser] updateCurrentUserWithFields:fieldsToUpdate withTrackingInformation:trackingInformation andLoginHandler:^(GTIOUser *user, NSError *error) {
            ...
            if (!error) {
                ...
                GTIOAlertView *alert = [[GTIOAlertView alloc] initWithTitle:nil message:@"We were not able to update your profile picture." delegate:self cancelButtonTitle:@"Okay" otherButtonTitles:nil];
                [alert show];
            }
        }];
    }];
...
{        
    ...
    [[GTIOUser currentUser] loadUserIconsWithCompletionHandler:^(NSArray *loadedObjects, NSError *error) {
        ...
        if (!error) {
            ...
            GTIOAlertView *alert = [[GTIOAlertView alloc] initWithTitle:nil message:@"We were not able to load your looks." delegate:self cancelButtonTitle:@"Okay" otherButtonTitles:nil];
            [alert show];
        }
    }];
...
- (void)connectToFacebook:(id)sender
{
    [[GTIOUser currentUser] connectToFacebookWithLoginHandler:^(GTIOUser *user, NSError *error) {
        if (!error) {
            ...
            GTIOAlertView *alert = [[GTIOAlertView alloc] initWithTitle:nil message:@"We were not able to connect your account to facebook." delegate:self cancelButtonTitle:@"Okay" otherButtonTitles:nil];
            [alert show];
        }
    }];
}
