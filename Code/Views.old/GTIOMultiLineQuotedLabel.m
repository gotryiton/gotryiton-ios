...
- (CGSize)calculateSizesAndDraw:(BOOL)draw inRect:(CGRect)rect {
	...
	for (int line = 0; line < maxLines; line++) {
		...
		if (line + 1 == maxLines || [partsLeft count] == 0) {
            ...
            if (draw) {
                [kGTIOColorBrightPink set];
                [@"”" drawAtPoint:CGPointMake(xAlignment-1+s.width,line*lineHeight-5) withFont:[UIFont systemFontOfSize:37]];
            }
            CGSize quoteSize = [@"”" sizeWithFont:[UIFont systemFontOfSize:37]];
            size = CGSizeMake(xAlignment-1+s.width+quoteSize.width,line*lineHeight+lineHeight);
			break;
		}
	}
    return size;
